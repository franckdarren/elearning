...
import { chapters, resources, sequences } from "@/lib/db/schema";
...
import { ScopeSelector } from "@/app/(teacher)/teacher/content/scope-selector";
import { ChapterDialog } from "@/app/(teacher)/teacher/content/chapter-dialog";
import { ConfirmDialog } from "@/components/shared/confirm-dialog";
import { deleteChapter, moveChapter } from "@/lib/actions/chapters";

export const metadata = { title: "Gestionnaire · Contenus" };
...
export default async function ManagerContentPage({
      ...
      <div className="space-y-6">
        ...
        <Card>
          <CardContent className="p-6 text-sm text-zinc-500">
            Aucune matière associée à une classe pour le moment. Créez des
            classes et matières, puis associez-les.
          </CardContent>
        </Card>
      </div>
    );
  ...
  const chosen = scope.find(
    (a) => a.classId === sp.classId && a.subjectId === sp.subjectId,
  );
  const active = chosen ?? scope[0];
