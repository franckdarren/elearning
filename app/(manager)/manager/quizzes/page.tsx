...
import { quizzes, classes, subjects, chapters, questions } from "@/lib/db/schema";
...
import { getEstablishmentScope } from "@/lib/queries/establishment-scope";
import { QuizDialog } from "@/app/(teacher)/teacher/quizzes/quiz-dialog";
import { QuizRowActions } from "@/app/(teacher)/teacher/quizzes/quiz-row-actions";

export const metadata = { title: "Gestionnaire · QCM" };
...
export default async function ManagerQuizzesPage() {
  const user = await requireEstablishment();
  const scope = await getEstablishmentScope(user.establishmentId);
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-zinc-500">{rows.length} quiz au total</p>
        </div>
        <QuizDialog
          assignments={scope}
          chapters={chaptersList}
          trigger={<Button disabled={scope.length === 0}>Nouveau quiz</Button>}
        />
      </div>
        ...
        <CardContent className="p-0">
          {rows.length === 0 ? (
            <p className="p-6 text-sm text-zinc-500">
              {scope.length === 0
                ? "Aucune matière associée à une classe pour le moment."
                : "Aucun quiz."}
            </p>
          ) : (
            <Table>
              ...
              <TableBody>
                {rows.map((q) => (
                  <TableRow key={q.id}>
                    <TableCell>
                      <Link
                        href={`/manager/quizzes/${q.id}/edit`}
