...
import { quizAttempts, quizzes, profiles, classes, subjects } from "@/lib/db/schema";
...
export const metadata = { title: "Gestionnaire · Résultats" };
...
export default async function ManagerResultsPage() {
  ...
  const [attempts, perQuiz] = await Promise.all([
    ...
    db
      ...
      .leftJoin(
        quizAttempts,
        and(eq(quizAttempts.quizId, quizzes.id), isNotNull(quizAttempts.submittedAt)),
      )
      .where(quizScope)
      .groupBy(quizzes.id)
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-zinc-500">
          Tentatives sur l&apos;ensemble des QCM de l&apos;établissement.
        </p>
      </div>
