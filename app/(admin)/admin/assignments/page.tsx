...
import { StudentScopeDialog } from "./student-scope-dialog";
import { deleteTeacherAssignment } from "@/lib/actions/assignments";

export const metadata = { title: "Admin · Affectations" };
export const dynamic = "force-dynamic";

export default async function AssignmentsPage() {
  const teacherProfiles = db
  ...
  // Teacher assignments with joins for display.
  const teacherAssignmentsList = db
    .select({
  ...
  // Map class -> [subjects] for the student dialog.
  const classWithSubjects = db
    .select({
      classId: classes.id,
      className: classes.name,
  ...
  // Student scope: each student's current class + granted subjects.
  const enrollmentsRows = db
    .select({
  ...
  for (const r of classSubjRows) {
    if (!classMap.has(r.classId)) {
      classMap.set(r.classId, { id: r.classId, name: r.className, subjects: [] });
    }
    if (r.subjectId && r.subjectName) {
      classMap.get(r.classId)!.subjects.push({ id: r.subjectId, name: r.subjectName });
    }
  }
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-zinc-500">
          Affecter les enseignants à des couples (classe + matière) et gérer
          l&apos;accès des élèves aux matières.
        </p>
      </div>
